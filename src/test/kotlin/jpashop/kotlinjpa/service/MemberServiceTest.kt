...
internal class MemberServiceTest {
	...
	fun `회원가입`() {
		...
		//then
		val findMember = memberRepo.findById(saveId)
			.get()
	...
	fun `중복 회원 예외`() {
		//given
		val member1 = Member(name = "조", address = Address("city", "street", "123"))

		val member2 = Member(name = "조", address = Address("city", "street", "123"))
		...
		//then
		//예외발생 해야함
		assertThatThrownBy { memberService.join(member2) }.isInstanceOf(IllegalArgumentException::class.java)
